...
class MUDClient:
    ...
    def login(self):
        ...
        # Check for menu and enter game - keep trying if needed
        max_attempts = 5
        for attempt in range(max_attempts):
            time.sleep(0.8)
            menu_check = self.receive(timeout=2)

            if "make your choice" in menu_check.lower() or "make your choice" in response.lower():
                print(f"Detected main menu (attempt {attempt+1}) - entering game (sending '1')...")
                self.send("1")
                time.sleep(2)
                game_response = self.receive(timeout=3)  # Wait for game to load
                if "welcome" not in game_response.lower() or "make your choice" not in game_response.lower():
                    print("Game entered successfully!")
                    return True
            elif "make your choice" not in menu_check.lower() and attempt > 0:
