...
- Running the full agent loop via `Boukensha.run`/`examples/example.rb` (as opposed to calling `Mcp::Client` directly) currently fails before making any tool call — the Anthropic API rejects the request with `400: system: Input should be a valid array`. Tracing the actual value on the wire: `system:` is sent as `nil`, not a plain string (the API accepts a plain string for `system` just fine; `nil` is neither a valid string nor an array, which is exactly what the error means). The `nil` traces back to `Config::PROMPTS_DIR` (`lib/boukensha/config.rb:13`, `File.expand_path("../../../prompts", __dir__)`) resolving to `week1_baseline/ruby/prompts` — one `..` too many, since the real file lives at `week1_baseline/ruby/10_standard_tool_library/prompts/system.md`. Because that path doesn't exist, `Tasks::Base.system_prompt` returns `nil`, `context.system` is `nil`, and `lib/boukensha/backends/anthropic.rb#to_payload` sends `system: nil` as-is. This is a pre-existing, one-line off-by-one bug in `Config::PROMPTS_DIR` (unrelated to MCP — the file predates this plan and no task touches it), not something this plan introduced or is in scope to fix. The MCP layer itself is proven working independent of this (see above); the backend bug blocks the full LLM-driven demo from completing and should be tracked separately.
