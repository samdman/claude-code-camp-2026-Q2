...
- [ ] **Blocker check — ask the user before proceeding past this point**: live verification against the real Anthropic API and the `mud-manager --mcp` server (`week0_explore/mud_manager`) requires (a) `ANTHROPIC_API_KEY` set, (b) a `.boukensha/settings.yaml` configured with a `tasks.player` entry and an `mcp_servers.mud` entry pointing at a runnable `mud-manager` binary (built via `week0_explore/mud_manager`'s gemspec), and (c) a reachable MUD server for `mud-manager` to connect to (per `docs/journal/week2.md`, this was `localhost:4000` in the prior environment, not present in this repo snapshot). Since this is a real, billed API call plus a dependency on infrastructure not confirmed present in this checkout, **stop and ask the user** whether to (1) skip live verification and consider the port done pending a separate manual test, (2) set up the missing pieces together, or (3) they already have a `.boukensha/settings.yaml` + running MUD server ready to point this at.
- [ ] If live verification proceeds: run `dotnet run --project week1_baseline/dotnet/src/Boukensha.Console -- --no-tui`, send one turn (e.g. "look around and tell me what you see"), and confirm: the REPL banner shows the correct provider/model/MCP servers, the turn completes without an unhandled exception, the session JSONL log at `<config dir>/sessions/*.jsonl` contains `session_start`, `prompt`, `tool_call`/`tool_result` (if the model calls a tool), and `response` events, and `Context.UsagePct` reflects a non-zero value after the turn.
- [ ] Update `docs/plans/week_2/dotnet_port.md`'s status line to reflect completion (or partial completion, noting what's pending) once this task concludes.
- [ ] Commit (final).
